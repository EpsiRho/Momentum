...
namespace Momentum.Models
{
    public class FileIndexer : INotifyPropertyChanged
    {
        ...
        {
            Dispatcher queue = q as Dispatcher;
            var dictionary = await SearchDirectory(Name);
        ...
        private async Task<Dictionary<string, List<IndexedFileInfo>>> SearchDirectory(string path)
        {
        ...
        public static async Task<bool> IsIndexAvailable()
        {
            try
            {
                ...
                return File.Exists($"{p}\\Momentum\\Index.json");
            }
            catch (Exception)
            {
                return false;
